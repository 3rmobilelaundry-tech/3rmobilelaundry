...
router.get('/analysis', async (req, res) => {
  try {
    if (req.user.role !== 'admin') return res.status(403).json({ error: 'Head Admin only' });
  ...
  try {
    ...
    if (!override) {
      ...
      await code.save();
       ...
       if (req.user.role !== 'admin' && req.user.role !== 'receptionist') return res.status(403).json({ error: 'Unauthorized' });
    }
