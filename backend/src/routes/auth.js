...
router.post('/login', async (req, res) => {
  try {
    ...
    if (!phone_number || !password) {
      return res.status(400).json({ error: 'Phone number and password are required', code: 'missing_credentials' });
    }
    if (phoneResult.error) {
      return res.status(400).json({ error: phoneResult.error, code: 'invalid_phone' });
    }
    
    const user = await User.findOne({ where: { phone_number: { [Op.in]: [phoneResult.normalized, phoneResult.local] } } });
    
    if (!user) {
      console.info('Login failed: user not found', { phone_number: phoneResult.normalized, ip: req.ip, ua: req.get('User-Agent') || 'unknown' });
      await AuditLog.create({
        actor_user_id: null,
        action: 'login_failed',
        entity_type: 'user',
        entity_id: `phone:${phoneResult.normalized}`,
